...
def matrix_decomposition(C,n_components,
                         normalise=True,
                         sign_flip=True,
                         do_migp=True,
                         algo='ICA',
    ...
    if algo == 'ICA':
        ...
        decomp = NMF(n_components=n_components, alpha_W=0.1, l1_ratio=1, init="nndsvd", random_state=1)
    else:
    ...
    # To get W:
    W = decomp.components_ #np.linalg.pinv(G)@C   #Note: this is W.T as it is more convenient to store W.T
...
def winner_takes_all(X, axis=1, z_thr=0.):
...
def save_W(W, ptx_folder, out_file):
    ...
    if sum(lut_vol.data.flatten()>0) != W.shape[1]:
        raise(Warning(f'Lookup_tractspace_fdt_matrix2 (size={sum(lut_vol.data.flatten()>0)} does not seem to be compatible with output W matrix (size={W.shape[1]})'))
...
def save_G(G, ptx_folder, out_file, seeds=None):
    ...
    coord_mat2 = np.loadtxt(os.path.join(ptx_folder,'coords_for_fdt_matrix2'),dtype=int)
    ...
    for idx, seed in enumerate(seeds):
        ...
        if is_gifti(seed):
            ...
            darrays = [nib.gifti.GiftiDataArray(data=np.array(x,dtype=float),
                                                datatype='NIFTI_TYPE_FLOAT32',
                                                intent=2001, meta=surf.darrays[0].meta) for x in G_seed.T]
...
class GLM():
    ...
    def calc_stats(self, C):
        if self.beta_ is not None:
            ...
            return dotdict({'tstat' : tstat, 'pval' : pval, 'zstat' : self.ttoz(tstat, self.dof_)})

    @staticmethod
    def ttoz(t,dof):
        def tdist(t,v):
            return betainc(v/2.,1/2,v/(v+t**2))/2.
