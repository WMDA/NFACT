...
class Cluster_parameters:
    ...
    def cluster_ram(self):
        ...
        self.arg["cluster_ram"] = self.arg["cluster_ram"] if self.arg["cluster_ram"] else 30

    def cluster_time(self):
        """Method to assign cluster time"""
        self.arg["cluster_time"] = (
            self.arg["cluster_time"] if self.arg["cluster_time"] else 160 if self.arg["gpu"] else 600
        )
    def cluster_queue_assignment(self):
        """Method to assign cluster queue"""
        self.arg["cluster_queue"] = (
            self.arg["cluster_queue"] if self.arg["cluster_queue"] in self.queues['std_out'] else None
        )
    ...
    def print_cluster_queues(self):
       ...
       print_string = "No queue given will assign" if not self.arg["cluster_queue"] else self.arg["cluster_queue"]
